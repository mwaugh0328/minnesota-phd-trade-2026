...
Note that this structure is similar to what Doireann talked about in class. And \citet{alvarez2007general} prove properties about the system defined by (\ref{eq:trade-balance}) and suggest an algorithm along the lines that Doireann described.
...
\begin{align}
\frac{d\mathcal{L}}{d \, d_{nj}} = & \underbrace{\int_0^1 \sum_k \frac{\partial \mathcal{L}}{\partial p_{nk}(\omega)} \frac{d p_{nk}(\omega)}{d \, d_{nj}} d\omega}_{\text{through prices}} + \underbrace{\int_0^1 \sum_k \frac{\partial \mathcal{L}}{\partial c_{nk}(\omega)} \frac{d c_{nk}(\omega)}{d \, d_{nj}} d\omega}_{\text{through quantities}} + \underbrace{\int_0^1 \sum_k \frac{\partial \mathcal{L}}{\partial x_{nk}(\omega)} \frac{d x_{nk}(\omega)}{d \, d_{nj}} d\omega}_{\text{through sourcing}}. \label{eq:ce-total-welfare}
\end{align}
So this says that the gains from trade relate to (i) the direct effect from a change in trade costs \textbf{through prices} and thus I'm allowing for GE forces to feed into here and then  (ii) indirect effects through how quantities change (iii) indirect effects about how sourcing patterns change.  Where we are going with this is the following |  all indirect effects vanish at the optimum and thus only direct effects matter.

This result may \textbf{not} be surprising. But as a history of thought and how some economists talk / talked about the new trade models it is surprising. In the EK framework specifically, people would say things like ...``well with an extensive margin on the sourcing decision, there will be ``new gains'' from trade.'' A sympathetic take is that one can kind of see this in the sense that if we were working with an Armington model, the sourcing effect would not be in equation (\ref{eq:ce-total-welfare}). However, this claim is not true (at least to a first-order) and it follows from basic economic theory that the consumer is optimizing over things. And this has nothing to do with the CES and Fr\'{e}chet assumption, in this sense the result I'm about to present is more general than in \citet{arkolakis2012new} paper.

Now let's first walk through the indirect effects are argue that they are zero.
...
\end{align}
Third, one might worry that there could be many ties that collectively matter. However, With continuous productivity distributions (as under the Fr\'{e}chet assumption), ties occur on a set of measure zero.\footnote{There is a third issue that some switches are occurring. However, these are precisely varieties that were near-indifferent. Hence the welfare effect from the discrete reallocation across sources is second order: the mass of switchers is $\mathcal{O}(\Delta)$ and the surplus that they could yield is $\mathcal{O}(\Delta)$ therefore the total affect of these forces are second-order $\mathcal{O}(\Delta^2)$.} Therefore:
\begin{align}
\int_0^1 \sum_{i,k} \frac{\partial \mathcal{L}}{\partial x_{ik}(\omega)} \frac{d x_{ik}(\omega)}{d \, d_{nj}} d\omega = 0.
\end{align}
Thus, changes in sourcing patterns have no indirect effects on welfare. This is the important indirect effect to rule out because it rules out that there are \textbf{no extra gains} from having an active extensive margin for where to source a good.

\textbf{Direct Effects Through Prices.} What I've argued is that the only source of gains from trade are direct effects. What remains is only the effect through prices:
\begin{align}
\frac{d\mathcal{L}}{d \, d_{nj}} = \int_0^1 \sum_k \frac{\partial \mathcal{L}}{\partial p_{nk}(\omega)} \frac{d p_{nk}(\omega)}{d \, d_{nj}} d\omega.
\end{align}
Then partial derivative of the Lagrangian with respect to prices is:
\begin{align}
\frac{\partial \mathcal{L}}{\partial p_{nk}(\omega)} = -\lambda_n x_{nk}(\omega) c_{nk}(\omega)
\end{align}
and thus
\begin{align}
\frac{d\mathcal{L}}{d \, d_{nj}} = -\lambda_n \int_0^1 \sum_k x_{nk}(\omega) c_{nk}(\omega) \frac{d p_{nk}(\omega)}{d \, d_{nj}} d\omega.
\end{align}
Now consider how prices change. The price is $p_{nk}(\omega) = \frac{w_k d_{nk}}{z_k(\omega)}$, so:
\begin{align}
\frac{d \ln p_{nk}(\omega)}{d \ln d_{nj}} = \begin{cases} 1 + \frac{d \ln w_j}{d \ln d_{nj}} & \text{if } k = j \\
 \frac{d \ln w_k}{d \ln d_{nj}} & \text{if } k \neq j \end{cases}
\end{align}
What we have then is
\begin{align}
\frac{d\mathcal{L}}{d \, d_{nj}}  =& \frac{-\lambda_n}{d_{nj}} \Bigg[ X_{nj}\bigg( 1 + \frac{d \ln w_j}{d \ln d_{nj}} \bigg)  + \sum_{k \neq j} X_{nk} \frac{d \ln w_k}{d \ln d_{nj}}\Bigg] \label{eq:quanity-form-welfare}
\end{align}
where $X_{nj} = \int_0^1 x_{nj}(\omega) p_{nj}(\omega) c_{nj}(\omega) \, d\omega$ is total expenditure on goods from $j$ and I inserted the elasticities (which was what allowed me to pull out a $p_{nj}(\omega)$ and construct expenditure.

This is nice, but there are a couple of issues to clear up to get things into the proper units. First, this is the change in utility which is not a meaningful number. But we can convert it to the ``dollars'' or what ever numeraire we have by dividing through by the marginal utility of wealth. Just as a recap to understand units here, $\lambda_n$ converts dollars into utils, i.e. how many utils do I get per dollar. Then $1 / \lambda_n$ converts utils into dollars.

Now one more observation, define $U_n$ as utility for the consumer in $n$ at the current equilibrium. Then $W_n \equiv \frac{1}{\lambda_n} U_n$ has the interpretation as the total number of dollars that are required to deliver $U_n$. We also know that the number of dollars to do so is given by $E_n$ which is total expenditure, thus $W_n = E_n$. So divide both sides by the ratio of trade costs to expenditure gives
\begin{align}
\frac{d \ln W_n}{d \ln d_{nj}} = -\pi_{nj}\left(1 + \frac{d \ln w_j}{d \ln d_{nj}}\right) - \sum_{k \neq j} \pi_{nk} \frac{d \ln w_k}{d \ln d_{nj}}
\end{align}
and this can be rewritten as:
\begin{align}
\frac{d \ln W_n}{d \ln d_{nj}} = -\pi_{nj} - \sum_{k} \pi_{nk} \frac{d \ln w_k}{d \ln d_{nj}} \label{eq:welfare-ge}
\end{align}
The welfare effect has two components. First there is the effect $\pi_{nj}$ which is the the share spent on goods from $j$ times the direct price reduction from lower trade costs. This is the (partial equilibrium) envelope result: a 1\% decrease in $d_{nj}$ increases real income by $\pi_{nj}$ percent. The welfare cost of lower trade costs is proportional to the expenditure share on the affected trade route.

This result on the first effect, turns out to have a super tight connection to standard consumer theory regarding the measurement of the welfare effects of change in prices. \citet{deaton1989rice} is a key reference with the insight that initial expenditure shares are the sufficient statistics needed to compute the welfare effects of price changes (in this case induced by a reduction in trade costs) to a first order.

Now there is a second GE effect $\sum_k \pi_{nk} \frac{d \ln w_k}{d \ln d_{nj}}$: a share-weighted sum of how all source prices change through wage adjustments. This is a ``leakage'' term where that word is sometimes used in public finance. Here leakage is represented by how other countries wages adjust given the shock. Or put another way, how the terms of trade adjust given the shock. Note if this country were small, then this term would be small.

One can show that the GE effects in \textbf{aggregate} net out to zero. In the appendix \ref{apx:ge-redistribution}, I show this. So if we look at welfare of other countries and then sum up these GE effects, globally they are zero and the only ``true'' gains from $d_{nj}$ are given by the share $\pi_{nj}$. Viewed in this light, these GE effects are simply redistributing the gains from trade across countries, and thus country $n$ does not recipe the full benefit of the reduction in $d_{nj}$.

Finally notice that CES or Fr\'{e}chet did not play a role in this argument. This is very general by arguing that only direct effects matter and those are characterized by expenditures and in GE how wages in other destinations respond.

\subsection{Hang On. What about \citet{arkolakis2012new}?}

At this point you should be totally confused. \citet{arkolakis2012new} told us one thing | only two sufficient statistics matter for the gains from trade. And I've said something else. How can they both be true. They are and let me show you how.

First, I'm going to derive a similar formula to (\ref{eq:welfare-ge}) but using ACRs formula. So we know that real income is
\begin{align}
...
\begin{align}
\pi_{nj} = \frac{(w_{j} d_{nj})^{-\theta}}{\sum\limits_{j' \in J}(w_{j'} d_{nj'})^{-\theta}}
\end{align}
Now lets compute the elasticity of $\frac{d \ln \pi_{nn}}{d \ln d_{nj}}$ under this assumption. Recall with the normalization that $w_n = 1$ we have
\begin{align}
\frac{d \ln \pi_{nn}}{d d_{nj}} = -\frac{d \ln \Phi_n}{d d_{nj}} \label{eq:acr-phi-change}
\end{align}
Then expand out $Phi_n$ and take the total derivative
\begin{align}
\frac{d \ln \Phi_n}{d \, d_{nj}} &= \frac{1}{\Phi_n} \Bigg[  \frac{-\theta (w_j d_{nj})^{-\theta}}{d_{nj}} + \sum_k \frac{-\theta (w_k d_{nk})^{-\theta}}{w_{k}} \frac{d w_k}{d \, d_{nj}} \Bigg]
\end{align}
then put in elasticity form by dividing through appropriately and noting that we have shares when the $Phi_n$ is multiplied through giving
\begin{align}
\frac{d \ln \Phi_n}{d \ln d_{nj}} = -\theta \pi_{nj} - \theta \sum_k \pi_{nk} \frac{d \ln w_k}{d \ln d_{nj}}
\end{align}
Then insert this into (\ref{eq:acr-phi-change}) and in turn (\ref{eq:acr-change})
\begin{align}
\frac{d \ln W_n}{d \ln d_{nj}} &= \frac{-1}{\theta} \times \Bigg[ -\theta \pi_{nj} - \theta \sum_k \pi_{nk} \frac{d \ln w_k}{d \ln d_{nj}} \Bigg] \nonumber  \\
\nonumber \\
&= -\pi_{nj} - \sum_{k} \pi_{nk} \frac{d \ln w_k}{d \ln d_{nj}} \label{eq:acr-deaton}
\end{align}
And we are back to my Deaton / Hulton -like result in (\ref{eq:welfare-ge}).  What is going on here? Here is my take
\begin{itemize}
  \item Equation (\ref{eq:welfare-ge}) is very general and says that only direct effects matter and those are characterized by the share of expenditure on the source plus share weighted GE effects. And the argument makes clear \textbf{why} a new margin like switching sources would not matter for the gains from trade (to a first order). CES or Fr\'{e}chet did not play a role in this argument.
...
\item Notice, that both my equation and ACR handle GE effects. However, one can see how the ACR and the demand system that they impose dramatically simplifies things when GE is considered | all of that stuff is baked into the home trade share. What this also means, per my discussion about the GE effects, is that the ACR gains \textbf{are not} a pure efficiency gains but also represent redistribution through the terms of trade from one country to another.

  \item An important point is local versus global. The ACR result is a global result, not first order. Whereas equation (\ref{eq:welfare-ge}) is local. This is an under-appreciated part of their result is the globally, different models are the same (near autarky, away etc.), and this is where the functional form assumptions deliver stronger results.
\end{itemize}
...
\section{GE wage effects are redistribution}\label{apx:ge-redistribution}

I'm now going to make the argument that in general equilibrium, the wage effects are pure redistribution across countries. What one country loses through leakage, the other gains.

I'm going to simplify things so we have two countries. Normalize $w_1 = 1$ as the numeraire. Trade balance requires:
\begin{align}
N_1\pi_{12}  = N_2\pi_{21} w_2.
\end{align}
From our earlier derivation, the welfare effects of a change in $d_{12}$ in \textbf{country 1} are
\begin{align}
\frac{d \ln W_1}{d \ln d_{12}} = -\pi_{12} - \pi_{12} \frac{d \ln w_2}{d \ln d_{12}}.
\end{align}
We can do the same thing for \textbf{country 2}
\begin{align}
\frac{d \ln W_2}{d \ln d_{12}} = \pi_{21} \frac{d \ln w_2}{d \ln d_{12}}.
\end{align}
Country 2 has receives no direct effect (the trade cost $d_{12}$ is the cost of shipping \emph{to} country 1). Its welfare changes only through the GE wage adjustment.

A key thing to resolve is that the formulas are in elasticity form. The leakage argument is in levels, so we need to say the level gain from one place nets off from a level loss from another. 

So country 1's welfare in levels:
\begin{align}
\frac{dW_1}{d \ln d_{12}} = W_1 \frac{d \ln W_1}{d \ln d_{12}} =   -N_1\pi_{12} - N_1 \pi_{12} \frac{d \ln w_2}{d \ln d_{12}}
\end{align}
where the last term is leakage in levels. Then country 2's welfare in levels:
\begin{align}
\frac{dW_2}{d \ln d_{12}} = W_2 \frac{d \ln W_2}{d \ln d_{12}} = N_2 \pi_{21}  w_2 \frac{d \ln w_2}{d \ln d_{12}}
\end{align}
which is all leakage. 

\textbf{Here is the important observation.} Use the trade balance condition $N_1\pi_{12}  = N_2 \pi_{21} w_2$ to rewrite country 2's gain
\begin{align}
\frac{dW_2}{d \ln d_{12}} =  \underbrace{N_1 \pi_{12} \frac{d \ln w_2}{d \ln d_{12}}}_{= \small\mbox{ -leakage from 1}}
\end{align}
These are exactly equal and opposite. What country 1 loses through the GE wage adjustment, country 2 gains. The direct effect $-\pi_{12}$ is the real resource saving from lower trade costs. The GE wage effects merely redistribute between the two countries, summing to zero. 
